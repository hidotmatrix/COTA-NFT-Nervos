import { Aggregator, FEE, generateIssuerInfoTx } from "@nervina-labs/cota-sdk";
...
const run = async () => {
  ...
  const issuer = {
    name: "Nervos Labs example",
    description: "Melting Two Wor.",
    avatar: "https://i.loli.net/2021/04/29/IigbpOWP8fw9qDn.png",
  };

  let rawTx = await generateIssuerInfoTx(
    service,
    cotaLock,
    issuer,
    FEE,
  ...
  console.info(
    `Set issuer information tx has been sent with tx hash ${txHash}`
  );
};
